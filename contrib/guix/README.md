# Bootstrappable Flocoin Core Builds

This directory contains the files necessary to perform bootstrappable Flocoin
Core builds.
...
To build Flocoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Flocoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/flocoin-detached-sigs> ./contrib/guix-codesign
```
...
repository](https://github.com/flocoin-core/guix.sigs).
