...
""" Interactive flocoind P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for Flocoin Core can enable. It uses BCC
...
def main(flocoind_path):
    peers = dict()

    flocoind_with_usdts = USDT(path=str(flocoind_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    flocoind_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    flocoind_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[flocoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/flocoind")
        exit()
    path = sys.argv[1]
    main(path)
