...
Tracing (USDT) support for Flocoin Core on Linux using. For more information on
USDT support in Flocoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `flocoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `flocoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Flocoin Core USDT documentation] on how to list available tracepoints in your
`flocoind` binary.

[Flocoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/flocoind
```
...
Flocoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/flocoind
```
...
In a different terminal, starting Flocoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/flocoind -signet -reindex
```
