...
Flocoin Core version 0.20.0 is now available from:

  <https://flocoincore.org/bin/flocoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Flocoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Flocoin Core on
unsupported systems.

From Flocoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Flocoin Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `FLOCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Flocoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Flocoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Flocoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Flocoin Core. Flocoin Core logs debug messages
...
- OpenSSL is no longer used by Flocoin Core.  (#17265)

- BIP70 support has been fully removed from Flocoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/flocoin/flocoin/issues/16599),
  [PR #16702](https://github.com/flocoin/flocoin/pull/16702), and the
  `flocoind help` for more information.  This option is experimental and
...
- All custom settings configured when Flocoin Core starts are now
...
- The "Start Flocoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://flocoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Flocoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Flocoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/flocoin-core/HWI) for
  signing.  Future versions of Flocoin Core should support a GUI option
...
- Flocoin Core's automatically-generated source code documentation is
  now available at https://doxygen.flocoincore.org.  (#17596)
...
- The `flocoin-cli` utility used with the `-getinfo` parameter now
...
- The `flocoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `flocoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_flocoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBFLOCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_flocoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBFLOCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for flocoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate flocoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest flocoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_flocoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (flocoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone flocoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Flocoin Core files (hebasto)
...
- #17119 Fix broken flocoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from flocoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.flocoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to flocoin-wallet (hel-o)
...
- #18341 Replace remaining literal FLO with `CURRENCY_UNIT` (domob1812)
...
- #16115 On flocoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from flocoin-cli -getinfo (malevolent)
