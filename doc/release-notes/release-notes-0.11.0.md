Flocoin Core version 0.11.0 is now available from:

  <https://flocoin.org/bin/flocoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer FLO/kB fee to be
...
For example, add the following to `flocoin.conf`:
...
related to the blockchain in the flocoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Flocoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Flocoin Core will 
...
For an example script see [flocoin-submittx](https://github.com/laanwj/flocoin-submittx).
...
- #5334 `cf87536` libflocoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new flocoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only FlocoinAmountField
- #5753 `0f44672` Add flocoin logo to about screen
...
- #5809 `46bfbe7` Add flocoin-cli man page
...
- #5849 `41259ca` contrib/init/flocoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Flocoin-Qt.app to Flocoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for flocoin-tx)
