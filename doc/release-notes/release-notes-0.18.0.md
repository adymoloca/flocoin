Flocoin Core version 0.18.0 is now available from:

  <https://flocoincore.org/bin/flocoin-core-0.18.0/>
...
`/Applications/Flocoin-Qt` (on Mac) or `flocoind`/`flocoin-qt` (on
Linux).
...
recommended to use Flocoin Core on unsupported systems.
...
10.10. Additionally, Flocoin Core does not yet change appearance when
...
`contrib/init/flocoind.service`, it has been changed to use
`/var/lib/flocoind` as the data directory instead of
`~flocoin/.flocoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/flocoind` will
exist has enough space (check using `df -h /var/lib/flocoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/flocoin/flocoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://flocoincore.org/en/contact/) the Flocoin Core
...
The systemd init file (`contrib/init/flocoind.service`) has been changed
to use `/var/lib/flocoind` as the data directory instead of
`~flocoin/.flocoin`. This change makes Flocoin Core more consistent with
...
`datadir` option in `/etc/flocoin/flocoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/flocoin/flocoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/flocoin/flocoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/flocoin/flocoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/flocoin/flocoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/flocoin/flocoin/blob/master/doc/flocoin-conf.md)
  about the `flocoin.conf` file describes how to use it to configure
  Flocoin Core.

- A new document introduces Flocoin Core's BIP174 [Partially-Signed
  Flocoin Transactions
  (PSBT)](https://github.com/flocoin/flocoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/flocoin/flocoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Flocoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Flocoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/flocoin/flocoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/flocoin/flocoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Flocoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Flocoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/flocoin/flocoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/flocoin/flocoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart flocoind with the `-deprecatedrpc=generate`
...
- A new `flocoin-wallet` tool is now distributed alongside Flocoin
...
This section describes planned changes to Flocoin Core that may affect
other Flocoin software and services.

- Since version 0.16.0, Flocoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Flocoin Core 0.20 (expected about
  a year after 0.18), Flocoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Flocoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Flocoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Flocoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Flocoin Core
...
- A sub-project of Flocoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Flocoin Core.  See their [project
  page](https://github.com/flocoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Flocoin Core's own implementation, although entropy
  gathered by Flocoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Flocoin Core a
...
- On macOS, Flocoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `flocoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libflocoin_server.a` (jnewbery)
...
- #13926 [Tools] flocoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like flocoind (MarcoFalke)
...
- #14521 qt, docs: Fix `flocoin-qt -version` output formatting (hebasto)
...
- #11625 Add FlocoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libflocoin_server` (laanwj)
...
- #15154 Configure: flocoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/flocoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Flocoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libflocoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in flocoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Flocoin Core (Christewart)
...
- #12246 Bugfix: Only run flocoin-tx tests when flocoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_flocoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_flocoin` (MarcoFalke)
...
- #14186 flocoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_flocoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update flocoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/flocoin-conf.md (hebasto)
...
- #15354 Add missing `flocoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct flocoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support flocoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/flocoin/).
