Flocoin Core version 0.14.0 is now available from:

  <https://flocoin.org/bin/flocoin-core-0.14.0/>
...
Flocoin Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/flocoin/flocoin/blob/master/doc/zmq.md) for
...
The Alert System was [disabled and deprecated](https://flocoin.org/en/alert/2016-11-01-alert-retirement) in Flocoin Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   ...
   optimizations to memory management. See [Pull #8753](https://github.com/flocoin/flocoin/pull/8753)
...
- #8722 `bb843ad` flocoin-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (flodrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnomflo)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnomflo)
...
- #8830 `ef0801b` test: Add option to run flocoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to flocoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to flocoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and flocoin-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (flodrak)
...
- #9691 `fc67cd2` Init ECC context for `test_flocoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change flocoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` flocoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow flocoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` flocoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to flocoin-tx (jnewbery)
