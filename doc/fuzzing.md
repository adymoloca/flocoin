# Fuzzing Flocoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Flocoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/flocoin/flocoin
$ cd flocoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/flocoin/flocoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/flocoin/flocoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/flocoin/flocoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/flocoin/flocoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`flocoin-core/qa-assets`](https://github.com/flocoin-core/qa-assets) repo.

To fuzz `process_message` using the [`flocoin-core/qa-assets`](https://github.com/flocoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/flocoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`flocoin-core/qa-assets`](https://github.com/flocoin-core/qa-assets) repo.

Every single pull request submitted against the Flocoin Core repo is automatically tested against all inputs in the [`flocoin-core/qa-assets`](https://github.com/flocoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Flocoin Core more robust.
...
with certain assembly code from Flocoin Core's code. See [developer notes on sanitizers](https://github.com/flocoin/flocoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Flocoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Flocoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Flocoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Flocoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Flocoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Flocoin
Core without having to write any custom fuzzing harness. The `flocoind` server
...
```sh
$ mkdir flocoin-honggfuzz-p2p/
$ cd flocoin-honggfuzz-p2p/
$ git clone https://github.com/flocoin/flocoin
$ cd flocoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/flocoind.cpp b/src/flocoind.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/flocoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
...
Flocoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/flocoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=flocoin-core).
...
because Flocoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_flocoin-core/latest).
