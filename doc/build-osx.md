...
This guide describes how to build flocoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Flocoin Core from source.
...
### 4. Clone Flocoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Flocoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `flocoind` or  `flocoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/flocoin/flocoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Flocoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/flocoin/flocoin/issues/7714).
...
You can deploy a `.dmg` containing the Flocoin Core application using `make deploy`.
...
There are many ways to configure Flocoin Core, here are a few common examples:
...
Run the following in your terminal to compile Flocoin Core:
...
## Running Flocoin Core

Flocoin Core should now be available at `./src/flocoind`.
If you compiled support for the GUI, it should be available at `./src/qt/flocoin-qt`.

The first time you run `flocoind` or `flocoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Flocoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Flocoin"

touch "/Users/${USER}/Library/Application Support/Flocoin/flocoin.conf"
...
./src/qt/flocoin-qt -server # Starts the flocoin-qt server mode, allows flocoin-cli control
```
