# User-space, Statically Defined Tracing (USDT) for Flocoin Core

Flocoin Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to Flocoin Core
...
Multiple tools can list the available tracepoints in a `flocoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Flocoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/flocoind
stap net        outbound_message 0x0000000000107c05 /src/flocoind
stap validation block_connected  0x00000000002fb10c /src/flocoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Flocoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/flocoind | grep NT_STAPSDT -A 4 -B 2
