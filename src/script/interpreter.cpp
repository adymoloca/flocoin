...
static bool EvalChecksigPreSigVerion(const valtype& vchSig, const valtype& vchPubKey, CScript::const_iterator pbegincodehash, CScript::const_iterator pend, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror, bool& fSuccess)
{
...
static bool EvalChecksig(const valtype& sig, const valtype& pubkey, CScript::const_iterator pbegincodehash, CScript::const_iterator pend, ScriptExecutionData& execdata, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror, bool& success)
{
    ...
    case SigVersion::WITNESS_V0:
        return EvalChecksigPreSigVerion(sig, pubkey, pbegincodehash, pend, flags, checker, sigversion, serror, success);
        break;
    }
    assert(false);
}
...
void PrecomputedTransactionData::Init(const T& txTo, std::vector<CTxOut>&& spent_outputs, bool force)
{
    ...
    for (size_t inpos = 0; inpos < txTo.vin.size() && !(uses_bip143_segwit); ++inpos) {
        if (!txTo.vin[inpos].scriptWitness.IsNull()) {
            if (m_spent_outputs_ready && m_spent_outputs[inpos].scriptPubKey.size() == 2 &&
        ...
        if (uses_bip143_segwit) break; // No need to scan further if we already need all.
    }
